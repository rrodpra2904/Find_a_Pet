...
// Conservación de datos para la Cartilla Veterinaria
$num_chip = "";
$vacuna_rabia = "0";
$desparasitado = "0";
$vacuna_nombre = "";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Recogida de datos sanitarios de la cartilla
    ...
    // Guardado de datos (Si todo está correcto)
    if ($todo_ok) {
        ...
        if (move_uploaded_file($temp, $carpeta . $foto)) {
            ...
            try {
                // Iniciao una transacción para asegurar que se guarde en todas las tablas o en ninguna
                $db->beginTransaction();

                // INSERTAR EN LA TABLA PRINCIPAL 'ANIMALES'
                ...
                // Recupero el ID autogenerado del animal que acabo de meter
                $nuevo_id_animal = $db->lastInsertId();

                // INSERTAR EN LA TABLA AUXILIAR 'FILTRO_ANIMALES'
                ...
                // INSERTAR EN LA TABLA AUXILIAR 'FILTRO_SEXO_ANIMALES'
                ...
                // INTEGRACIÓN CLAVE: INSERTAR EN 'TABLON_DE_ADOPCIONES' CON SU CARTILLA VETERINARIA COMPLETA
                // Fuerzo campos por defecto como 'administrador' y verificado '1' según se muestra en tu estructura
                ...
                // Vinculo todas las columnas sanitarias rellenadas por el Administrador
                ...
                // Confirmar la transacción limpia
                $db->commit();
                ...
                // Si algo falla, cancelo todo el proceso para evitar descuadres en la base de datos
                $db->rollBack();
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Contenedor especial para los switches o elementos de la cartilla */
